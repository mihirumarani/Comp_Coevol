...
network<-function(a,omega,t){
  ...
  for(i in samples){
    for(j in samples){
      ...
      mat[i,j]<-alpha.gt(a[i],a[j],omega,t)
...
#Load raw data
tot<-readRDS("SK_GT1020_55tr.Rds")
...
for(i in 1:nrow(meandat)){
  ...
  for(j in 1:length(mat.l)){
    means<-c(means,apply(mat.l[[j]],1,function(x) sum(x*geno)))
  }
  meandat[i,]<-means
}

#Calculate trait variances with time
vardat<-matrix(nrow=nrow(param),ncol=nsp*length(samples))
tps<-rep(1:length(samples),each=nt)
for(i in 1:nrow(vardat)){
  ...
  vars<-c()
  for(j in 1:length(mat.l)){
    vars<-c(vars,apply(mat.l[[j]],1,function(x) (sum((x*(geno^2)))-((sum(x*geno))^2))))
  }
  vardat[i,]<-vars
}
...
for(i in 1:nrow(meandat)){
  for(j in 1:length(samples)){
    modat[i,j]<-mods(meandat[i,which(tsteps==j)],param[i,2],param[i,1])
  }
}
...
colnames(mnnds1)<-c("t","Omega","inter",samples)
...
plot2dat<-melt(mnnds1,id=c("t", "Omega","inter"))
...
plot2datm<-aggregate(plot2dat$value,list(plot2dat$t,plot2dat$Omega,plot2dat$inter,plot2dat$variable),mean,na.rm=TRUE)
plot2dats<-aggregate(plot2dat$value,list(plot2dat$t,plot2dat$Omega,plot2dat$inter,plot2dat$variable),sd,na.rm=TRUE)
plot2dat1<-cbind(plot2datm,plot2datm$x+plot2dats$x,plot2datm$x-plot2dats$x)
colnames(plot2dat1)<-c("t","omega","inter","Time","mean","upper","lower")
...
colnames(exdat2)<-c("t","Omega","inter",samples)
exdat2<-as.data.frame(exdat2)
exdat2$t<-as.factor(exdat2$t)
exdat2$inter<-as.factor(exdat2$inter)
exdat2$Omega<-as.factor(exdat2$Omega)

plot2dat<-melt(exdat2,id=c("t","Omega","inter"))
plot2dat$variable<-as.numeric(plot2dat$variable)
dumb<-function(i){return(samples[i])}
plot2dat$variable<-sapply(plot2dat$variable,dumb)
...
t.labs<-c("Threshold=1","Threshold=2.5","Threshold=5","No Threshold")
names(t.labs)<-c("1","2.5","5","100")
...
plot2<-ggplot(plot2dat1,aes(x=Time,y=mean, group=omega,colour=omega)) +geom_line(size=1.5) +
  geom_ribbon(aes(ymin=lower, ymax=upper, x=Time, fill=omega),alpha=0.2)+ylab("No. of extant species")+
  facet_grid(t~inter,labeller=labeller(t=t.labs,inter=i.labs),switch="y")+
  theme(
    strip.text.x = element_text(
      size = 15, face = "bold"
